...
public class CrawlService {
    ...
    public CrawlService injectUrls(Set<Cookie> cookies, String... urls) {
        ...
        logger.debug("Prepare to add {} urls to lab.s2jh.crawl queue.", urls.length);
        for (String url : urls) {
            try {
                url = url.trim();
                logger.info("Injected lab.s2jh.crawl url: {}", url);
                logger.info("CrawlTaskExecutor ActiveCount/PoolSize/MaxPoolSize: {}/{}/{}",
                        crawlTaskExecutor.getActiveCount(), crawlTaskExecutor.getPoolSize(),
                        crawlTaskExecutor.getMaxPoolSize());
                ...
                //Just logger error to continue next url lab.s2jh.crawl
    ...
    public void stopCrawl() {
        logger.info("Stop lab.s2jh.crawl signal received.");
        stopUrlInject = true;
    }
